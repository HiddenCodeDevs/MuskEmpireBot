            ...
            if success:
                ...
                for fnd in response_json['data']['funds']:
                    if fnd['fundKey'] == fund:
                        ...
                        money_str = f"Profit: +{money}" if money > 0 else (
                            f"Loss: {money}" if money < 0 else "Profit: 0")
        ...
        async with aiohttp.ClientSession(headers=headers, connector=proxy_conn) as http_client:
            ...
            while True:
                ...
                try:
                    ...
                    unrewarded_quests = [quest['key'] for quest in full_profile['data']['quests'] if
                                         not quest['isRewarded']]
                    ...
                    unrewarded_friends = [int(friend['id']) for friend in full_profile['data']['friends'] if
                                          friend['bonusToTake'] > 0]
                        ...
                        if self.dbs:
                            ...
                            if league_data is not None:
                                if self.level >= int(league_data['requiredLevel']):
                                    self.strategies = [strategy['key'] for strategy in
                                                       self.dbs['dbNegotiationsStrategy']]
                                    if config.PVP_STRATEGY == 'random' or config.PVP_STRATEGY in self.strategies:
                                        await self.perform_pvp(league=league_data, strategy=config.PVP_STRATEGY,
                                                               count=config.PVP_COUNT)
                                    else:
                                        config.PVP_ENABLED = False
                                        log.warning(
                                            f"{self.session_name} | PVP_STRATEGY param is invalid. PvP negotiations disabled.")
                                else:
                                    config.PVP_ENABLED = False
                                    log.warning(
                                        f"{self.session_name} | Your level is too low for the {config.PVP_LEAGUE} league. PvP negotiations disabled.")
                            else:
                                config.PVP_ENABLED = False
                                log.warning(
                                    f"{self.session_name} | PVP_LEAGUE param is invalid. PvP negotiations disabled.")
                        else:
                            log.warning(
                                f"{self.session_name} | Database is missing. PvP negotiations will be skipped this time.")
